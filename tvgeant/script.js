// script.js
 ...
 buttons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // تحديث المظهر البصري للأزرار
      buttons.forEach(b => b.classList.remove('border-orange-500', 'text-orange-500', 'font-bold'));
      btn.classList.add('border-orange-500', 'text-orange-500', 'font-bold');
    });
  });
    ...
    .catch(err => {
      ...
      function updateHomeDeliveryPrice() {
        ...
        if (wilaya) {
          ...
          homeDeliveryPriceSpan.textContent = cost > 0 ? `${cost.toFixed(2)}دج` : '-';
        } else {
  ...
  const subTotal = baseAmount + deliveryCost;

  // ✅ السعر الإجمالي = السعر الفرعي × الكمية
  const total = baseAmount * quantity + deliveryCost;

  // ✅ تحديث العرض
  subTotalEl.textContent = `${subTotal.toFixed(2)} دج`;
  totalAmountEl.innerHTML = `${total.toFixed(2)} <span class="text-base font-normal">دج</span>`;
         // ✅ تحديث الحقل المخفي
        // ✅ تحديث الحقول المخفية
  document.getElementById('totalPriceInput').value = total.toFixed(2);
  document.getElementById('deliveryPriceInput').value = deliveryCost.toFixed(2);
